...
export function Curfew({ onSupport }: { onSupport?: (key: string) => void } = {}) {
  ...
  // A manual refund goes through wv's gate: `wv support refund <pay_id>` reads the payment first and shows what
  // was already refunded before anything runs. Curfew's own automatic actions stay on its server.
  const [hasWv, setHasWv] = useState(false);
  useEffect(() => { wvPath().then((x) => setHasWv(!!x)); }, []);
  const [refund, setRefund] = useState<ActionSpec | null>(null);
  const refundVia = (p: Payment) => setRefund({ key: `curfew.refund.${p.id}`, title: `Refund ${money(p.usd_total)} to ${p.user_name || "this customer"}`, description: "wv reads the payment, shows what was already refunded and what remains, and refunds only after you approve the plan.", fields: [], build: () => ["support", "refund", p.id] });
  ...
  return <div className="stack curfew-page">
    ...
    {selected && <Dialog.Root open onOpenChange={o=>{if(!o)setSelected(null);}}><Dialog.Content style={{maxWidth:500}}><Dialog.Title>Payment details</Dialog.Title><Dialog.Description>{selected.user_name || "Unknown customer"} · {money(selected.usd_total)}</Dialog.Description><div className="curfew-payment-detail"><Badge color={(selected.risk??0)>=70 ? "red" : "gray"}>Risk score {selected.risk ?? "—"} / 99</Badge><Text size="1" color="gray">This is a rules-based score, not a probability of fraud.</Text>{selected.insights?.map((t,i)=><p key={i}>{t}</p>)}<Text size="1" color="gray">{selected.id} · {new Date(selected.created_at).toLocaleString()}</Text></div><div className="workspace-dialog-actions">{hasWv&&onSupport&&<Button variant="soft" onClick={()=>{onSupport(selected.id);setSelected(null);}}>Open in Support</Button>}{hasWv&&selected.status==="paid"&&!selected.refunded_at&&<Button variant="soft" color="amber" onClick={()=>{refundVia(selected);setSelected(null);}}>Refund with approval</Button>}<Button onClick={()=>setSelected(null)}>Done</Button></div></Dialog.Content></Dialog.Root>}
    {refund && <ActionEditor key={refund.key} spec={refund} onClose={()=>setRefund(null)}/>}
  </div>;
}
