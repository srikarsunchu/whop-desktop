...
function DraftCard({draft,onSave,ask}:{draft:StudioAdDraft;onSave:(d:StudioAdDraft)=>void;ask:(prompt:string)=>void}){
  ...
  const missing=[!draft.context.adHeadline.trim() && 'headline',!draft.context.postCopy.trim() && 'caption',!validDestination(draft.context.destinationUrl) && 'destination'].filter(Boolean);
